...
class WReN:
    ...
    def __init__(self, conc_unit = unyt.mg/unyt.kg, flow_unit = unyt.kg/unyt.s, GUI_terminal = None):
        ...
        self.processes = pd.Series(dtype = object)
    ...
    def add_process(self, sink_conc, source_conc, sink_flow, source_flow = None, process_name = None, conc_unit = None, flow_unit = None, conc_names = None, GUI_oe_tree = None):
        ...
        if 'costs' in dir(self):
            ...
            self.costs = pd.concat([self.costs, temp])
            self.costs.iloc[:, -1] = 0.0
            self.upper = pd.concat([self.upper, temp])
            self.upper.iloc[:, :] = -1
            self.lower = pd.concat([self.lower, temp])
    ...
    def solve_WReN(self, water_costs = None, upper = None, lower = None):
        """
        ...
        if upper is None or np.all(upper == -1): # Automatically set the upper limits
            ...
            raise ValueError('Upper must be a {len(self.processes[self.active_processes]) + 1}x{len(self.processes[self.active_processes]) + 1} matrix')
        ...
        if lower is None:
            ...
            raise ValueError(f'Lower must be a {upper.shape[0]}x{upper.shape[1]} matrix')
        ...
        if water_costs is None:
            water_costs = self.costs
        water_costs = water_costs.values.flatten()[1:] # [1:] because FW to WW is always 0, so that entry isn't in the solver function
        def objective(x, water_costs):
            return (x**0.6).dot(water_costs)
        cost_fun = lambda x: objective(x, water_costs)

        options = {'maxiter': 5000, 'eps': 1e-10, 'ftol': 1e-3} # Options for the solver
        n_cpus = cpu_count()
        multicore_results = Parallel(n_jobs = n_cpus)(delayed(_solver_for_joblib)(self.processes, self.active_processes,
                cost_fun, bounds, cons, options, cons_len, sink_flow, source_flow, cutoff_tol) for _ in range(n_cpus))
        ...
        temp_costs = np.array([cost_fun(elem.flatten()[1:]) for elem in multicore_results if np.all(elem != None)])
...
def _solver_for_joblib(processes, active_processes, cost_fun, bounds, cons, options, cons_len, sink_flow, source_flow, cutoff_tol): # processes and active_processes
    """
    ...
    for x0 in starting_points:
        ...
        if 'best_objective' not in locals() or mysol['fun'] < best_objective:
            ...
            # Eqn 2: For each contaminant, contaminant mass balance for sinks
            for contam_idx in range(len(processes.iat[0].sink_conc)):
                source_conc = np.array([proc.source_conc.iloc[contam_idx].value for proc in processes[active_processes]])
                sink_conc = np.array([proc.sink_conc.iloc[contam_idx].value for proc in processes[active_processes]])
